package fr.mgargadennec.blossom.autoconfigure.ui.web.administration.user;
...
public class WebAdministrationUserAutoConfiguration {
  ...
  public MenuItem administrationUserMenuItem(MenuItemBuilder builder, @Qualifier("administrationMenuItem") MenuItem administrationMenuItem) {
    return builder.key("users").label("menu.administration.users", true).link("/blossom/administration/users").icon("fa fa-user").parent(administrationMenuItem).build();
  }
