...
<form  class="form form-horizontal" onsubmit="submit_user_roles_associations(this);return false;">
  <div class="ibox-content">
    ...
    <div class="row">
      <select id="association-selector" name="ids" multiple>
        <#list roles as role>
          <option value="${role.id?c}" <#if isAssociated(associations, user.id, role.id)>selected</#if>>${role.name}</option>
        </#list>
      </select>
    </div>
  </div>
  ...
  <div class="ibox-footer">
    <div class="text-right">
      <button class="btn btn-default btn-view" type="button" onclick="cancel_user_roles_associations(this);">
...
<script>
  ...
  var submit_user_roles_associations = function (button) {
  ...
  var cancel_user_roles_associations = function (button) {
    var targetSelector = '#'+$(button).closest(".tab-pane").attr('id');
    $(targetSelector + ' > .ibox-content').addClass("sk-loading");
