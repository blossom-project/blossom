...
<#function isAssociated associations userId groupId>
  <#list associations as association>
    <#if association.a.id == userId && association.b.id == groupId>
      <#return true/>
    </#if>
  </#list>
  <#return false/>
...
<form  class="form form-horizontal" onsubmit="submit_group_users_associations(this);return false;">
  <div class="ibox-content">
    ...
    <div class="row">
      <select id="association-selector" name="ids" multiple>
        <#list users as user>
          <option value="${user.id?c}" <#if isAssociated(associations, user.id, group.id)>selected</#if>>${user.firstname} ${user.lastname} (${user.identifier})</option>
        </#list>
      </select>
    </div>
  </div>
  ...
  <div class="ibox-footer">
    <div class="text-right">
      <button class="btn btn-default btn-view" type="button" onclick="cancel_group_users_associations(this);">
...
<script>
  ...
  var submit_group_users_associations = function (button) {
  ...
  var cancel_group_users_associations = function (button) {
    var targetSelector = '#'+$(button).closest(".tab-pane").attr('id');
    $(targetSelector + ' > .ibox-content').addClass("sk-loading");
