...
<div id="status" class="ibox float-e-margins">
  ...
  <div class="ibox-content">
    ...
    <h2 class="global <#if health.getStatus().getCode() == 'UP'>text-navy<#else>text-danger</#if>">
      ...
      <@spring.messageText "dashboard.server.status."+health.getStatus().getCode() health.getStatus().getCode() />
    </h2>
    ...
    <table class="table table-stripped small m-t-md">
      ...
      <tr>
        <td
          class="<#if health.getDetails()[key].getStatus().getCode() == 'UP'>text-navy<#else>text-danger</#if>">
          <i class="fa fa-play <#if health.getDetails()[key].getStatus().getCode() == 'UP'>fa-rotate-270<#else>fa-rotate-90</#if>"></i>
          <@spring.messageText "dashboard.server.status."+health.getDetails()[key].getStatus().getCode() health.getDetails()[key].getStatus().getCode() />
        </td>
        <td>
        ${key}
        </td>
