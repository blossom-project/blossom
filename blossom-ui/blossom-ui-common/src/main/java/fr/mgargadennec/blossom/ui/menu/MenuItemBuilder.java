...
public class MenuItemBuilder {
  ...
  public MenuItemBuilder parent(String parentKey) {
    this.parent = this.registry.getPluginFor(parentKey, new RuntimeException("Cannot find menu item with key " + parentKey + ". Please check your configuration order !"));
    checkLevel();
    return this;
  }
  ...
  private void checkLevel() {
    if (this.parent != null && this.parent.level() >= maxDepth) {
      throw new RuntimeException("Cannot add MenuItem to parent (key = " + parent.key() + ", level=" + parent.level() + ") because max depth is " + maxDepth);
    }
  }


  public MenuItem build() {
    return new MenuItemImpl(registry, key, icon, label, i18n, link, parent);
  }
  ...
  public static class MenuItemImpl implements MenuItem {
    ...
    MenuItemImpl(PluginRegistry<MenuItem, String> registry, String key, String icon, String label, boolean i18n, String link, MenuItem parent) {
      this.registry = registry;
      this.key = key;
      this.icon = icon;
      this.label = label;
    ...
    public Collection<MenuItem> items() {
      return this.registry.getPlugins().stream().filter(item -> item.parent() != null && item.parent().key().equals(this.key)).collect(Collectors.toList());
    }
