...
public class FilterHandlerMethodArgumentResolver implements FilterArgumentResolver {
  ...
  private static final Splitter itemSplitter= Splitter.on("/").trimResults().omitEmptyStrings();
  private static final Splitter equalsSplitter= Splitter.on("//").trimResults().omitEmptyStrings();
  private static final Splitter semicolonSplitter= Splitter.on(";").trimResults().omitEmptyStrings();
  ...
  private Optional<QueryBuilder> parseAndApplyBoundaries(String filterString) {
    ...
    semicolonSplitter.splitToList(filterString).forEach(
      filter -> {
        List<String> filterData = equalsSplitter.splitToList(filter);
        String filterName = filterData.get(0);
        if(filterData.size() == 2){
          List<String> filterValues = itemSplitter.splitToList(filterData.get(1));
